...
export default function AuthCodePanel({ variant = "login" }) {
  const w = useWidth();
  // Hide the left panel entirely on mobile, show on tablet+
  ...
  return (
    <div style={{
      ...
      minHeight: "100vh",
      background: "#0A0A0F",
      padding: w >= 1024 ? "40px 48px" : "32px 32px",
      display: "flex",
      ...
      {/* Logo */}
      <div style={{ marginBottom: w >= 1024 ? 64 : 40 }}>
        <Logo />
      </div>
      ...
      <div style={{ marginBottom: 40 }}>
        <div style={{
          ...
          {/* Code lines */}
          <div style={{ padding: "14px 14px 14px 0", overflowX: "auto" }}>
            {lines.map((line, i) => (
              <div key={i} style={{ display: "flex", marginBottom: 4, minHeight: 22 }}>
                ...
                <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 12, lineHeight: "22px", whiteSpace: "nowrap" }}>
                  {line.map((tk, j) => (
      ...
      <div style={{
        ...
        <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#475569", letterSpacing: "0.08em" }}>
